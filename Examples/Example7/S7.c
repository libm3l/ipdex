 ...
 */
     if (argc < 4) {
       fprintf(stderr,"ERROR, no IPaddress and port number2 provided\n");
       exit(1);
     }
 	portno1 = atoi(argv[2]);
 	portno2 = atoi(argv[3]);
 ...
 */
		Gnode = client_name("Text from Solver_7");
// 	
 ...
 */	
		dim[0] = 4;
		if(  (TmpNode = m3l_Mklist("S7_numbers", "I", 1, dim, &Gnode, "/Client_Data", "./", (char *)NULL)) == 0)
				Error("m3l_Mklist");
		tmpdf = (lmint_t *)m3l_get_data_pointer(TmpNode);
		for(j=0; j<4; j++)
			tmpdf[j] = (i+1)*j*3;
		free(dim);
