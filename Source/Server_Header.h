...
typedef struct SR_thread_args{
	...
	lmint_t 		*psockfd, *pEofBuff;       /* socket id, unique to every thread, can be in thread stack, End-ofbuffer signal */
...
typedef struct SR_thread_str{
	...
	pthread_t 		*data_threads;	/* thread ID of all threads in group data_threads */
	lmint_t  		*sockfd, *EofBuff;               /* socket id, unique to every thread, can be in thread stack , End-ofbuffer signal */
